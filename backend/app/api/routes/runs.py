...
async def get_run(run_id: int, current_user: CurrentUser):
    async with AsyncSessionLocal() as session:
        ...
        return {
            ...
            "created_at": r.created_at.isoformat() if run.created_at else None,
        }
...
async def get_playlist_items(
    ...
    """Get recommendation items for a specific playlist with match status."""
    async with AsyncSessionLocal() as session:
        # Verify run and playlist
...
async def get_playlist(playlist_id: int, current_user: CurrentUser):
...
async def get_playlist_items_by_id(
    ...
    """Get recommendation items for a playlist (direct ID lookup)."""
    return await get_playlist_items(playlist_id, playlist_id, current_user, limit, offset)