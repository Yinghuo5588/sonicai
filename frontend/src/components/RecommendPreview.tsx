  ...
  return (
    <div className="bg-blue-50 rounded-lg p-3 border border-blue-100 space-y-2 text-sm">
      <div className="flex justify-between">
        <span className="text-slate-600">理论获取</span>
        <span className="font-medium text-slate-800">{theoreticalMax} 首</span>
      </div>
      <div className="flex justify-between">
        <span className="text-slate-600">候选池</span>
        <span className="font-medium text-slate-800">{candidatePool} 首</span>
      </div>
      <div className="flex justify-between">
        <span className="text-slate-600">最低命中</span>
      ...
      {wasteLevel === '严重' && (
        <p className="text-xs px-2 py-1 rounded bg-red-50 text-red-600">
          候选池仅占获取量的 {wastePercent}%，大量相似曲目将被丢弃，建议调高推荐平衡或增大歌单大小
        </p>
      )}
      {wasteLevel === '提醒' && (
        <p className="text-xs px-2 py-1 rounded bg-yellow-50 text-yellow-700">
