...
export default function RunDetailPage() {
  ...
  if (!run) return <div className="p-6 text-red-500">任务不存在</div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-4">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-lg font-bold text-slate-800">
            {run.run_type === 'full' ? '完整推荐' : run.run_type === 'similar_tracks' ? '相似曲目' : '相邻艺术家'}
          </h1>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                  pl.status === 'success' ? 'bg-green-100 text-green-700' :
                  pl.status === 'failed' ? 'bg-red-100 text-red-700' :
                  'bg-slate-100 text-slate-600'
                }`}>
                  {pl.status}
                </span>
