...
function statusBadge(status: string) {
  const cls = status === 'success' ? 'badge-success' : status === 'failed' ? 'badge-danger' : status === 'running' ? 'badge-info' : status === 'stopped' ? 'badge-warning' : 'badge-muted'
...
export default function RunDetailPage() {
  ...
  if (runLoading) return <div className="p-4 text-slate-500">加载中...</div>
  if (runError) return <div className="p-4 text-red-500">加载失败：{(runError as Error).message}</div>
  if (!run) return <div className="p-4 text-slate-500">任务不存在</div>

  return (
    <div className="page">
      {/* Back */}
      <div className="flex items-center justify-between">
        <Link to="/history" className="text-sm text-blue-500 hover:underline dark:text-blue-400">← 推荐历史</Link>
      ...
      <div className="card card-padding">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-lg font-bold text-slate-900 dark:text-slate-50">{runTypeLabel(run.run_type)}</h1>
          {statusBadge(run.status)}
        </div>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 space-x-1">
          <span>创建于 {formatRelativeTime(run.created_at)}</span>
          {run.started_at && <span>· 开始于 {formatDateTime(run.started_at)}</span>}
          {run.finished_at && <span>· 完成于 {formatDateTime(run.finished_at)}</span>}
        </p>
        {run.error_message && (
          <p className="text-xs text-red-500 mt-2 bg-red-50 dark:bg-red-950/50 rounded-lg p-3">{run.error_message}</p>
        )}
      </div>
      ...
      {run.progress && (
        <div className="card card-padding">
          <div className="flex justify-between text-sm mb-2">
            <span className="text-slate-500 dark:text-slate-400">匹配进度</span>
      ...
      <div>
        ...
        {playlists && playlists.map((pl: any) => (
          <Link
            ...
            className="card card-padding flex items-center justify-between mb-2 hover:bg-slate-50 dark:hover:bg-slate-900 transition"
          >
